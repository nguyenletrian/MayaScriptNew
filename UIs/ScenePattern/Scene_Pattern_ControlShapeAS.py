...
def Form(data,*arr):
    ...
    def QuickExportCurve(data,*arr):
        ...
        if selection:
            ...
            allAsCtrlTemp = cmds.ls(
                type="nurbsCurve",
                ap=True
            )

            for curve in allAsCtrlTemp:
        ...
        for ctrl in allAsCtrl:
            ...
            data_temp[ctrl]["overrideEnabled"] = cmds.getAttr(
                ctrl + ".overrideEnabled"
            )

            data_temp[ctrl]["overrideRGBColors"] = cmds.getAttr(
                ctrl + ".overrideRGBColors"
            )

            data_temp[ctrl]["visibility"] = cmds.getAttr(
                ctrl + ".visibility"
            )

            if data_temp[ctrl]["overrideRGBColors"] == 1:

                data_temp[ctrl]["overrideColorR"] = cmds.getAttr(
                    ctrl + ".drawOverride.overrideColorR"
                )

                data_temp[ctrl]["overrideColorG"] = cmds.getAttr(
                    ctrl + ".drawOverride.overrideColorG"
                )

                data_temp[ctrl]["overrideColorB"] = cmds.getAttr(
                    ctrl + ".drawOverride.overrideColorB"
                )

            else:

                data_temp[ctrl]["overrideColor"] = cmds.getAttr(
                    ctrl + ".overrideColor"
                )
            ...
            if curveShapes:
                ...
                for curveChild in curveShapes:

                    if "Orig" in curveChild:
                        continue
                    ...
                    curveData[curveChildName]["degree"] = cmds.getAttr(
                        curveChild + ".degree"
                    )

                    curveData[curveChildName]["form"] = cmds.getAttr(
                        curveChild + ".form"
                    )
                    ...
                    points = cmds.ls(
                        curveChild + ".controlPoints[*]",
                        flatten=True
                    )

                    for point in points:

                        if not cmds.objExists(point):
                            continue

                        pointName = point.split("|")[-1]
                        pointName = pointName.split(".")[-1]

                        pointData[pointName] = cmds.xform(
                            point,
                            q=True,
                            os=True,
                            t=True
                    ...
                    curveData[curveChildName]["degree"] = fnCurve.degree
                    curveData[curveChildName]["form"] = int(fnCurve.form)

                    knots = fnCurve.knots()

                    curveData[curveChildName]["knots"] = [
                        float(knot)
                        for knot in knots
                    ]
                    ...
                    curveData[curveChildName]["overrideEnabled"] = cmds.getAttr(
                        curveChild + ".overrideEnabled"
                    )

                    curveData[curveChildName]["overrideRGBColors"] = cmds.getAttr(
                        curveChild + ".overrideRGBColors"
                    )

                    curveData[curveChildName]["visibility"] = cmds.getAttr(
                        curveChild + ".visibility"
                    )

                    if curveData[curveChildName]["overrideRGBColors"] == 1:

                        curveData[curveChildName]["overrideColorR"] = cmds.getAttr(
                            curveChild + ".drawOverride.overrideColorR"
                        )

                        curveData[curveChildName]["overrideColorG"] = cmds.getAttr(
                            curveChild + ".drawOverride.overrideColorG"
                        )

                        curveData[curveChildName]["overrideColorB"] = cmds.getAttr(
                            curveChild + ".drawOverride.overrideColorB"
                        )

                    else:

                        curveData[curveChildName]["overrideColor"] = cmds.getAttr(
                            curveChild + ".overrideColor"
                        )
            ...
            if ctrl == "HipSwinger_M":
                ...
                offsetGroup = cmds.listRelatives(
                    ctrl,
                    parent=True
                )[0]
        ...
        file_path = data["sceneDataPath"]+ "dataCurveShape.json"
    ...
    def RebuildCurveShape(ctrl, curveChild, curveData):
        """
        Rebuild a nurbsCurve shape under an existing controller.

        The controller transform itself is preserved.
        """

        pointData = curveData.get("pointData", {})
        ...
        degree = curveData.get("degree", 3)
        form = curveData.get("form", 0)
        knots = curveData.get("knots", [])
        ...
        for pointName, position in sortedPoints:
            points.append(position)

        if len(points) < degree + 1:
            print(
                "Cannot rebuild curve '{}': not enough CVs.".format(
                    curveChild
                )
            )
            return None
        ...
        try:
            ...
            # Maya cmds.curve can recreate the shape from CVs.
            tempCurve = cmds.curve(
                degree=degree,
                point=points,
                name=ctrl + "_TempCurve"
            )

            tempShape = cmds.listRelatives(
                tempCurve,
                shapes=True,
                fullPath=True
            )[0]
            ...
            oldShapePath = ctrl + "|" + curveChild

            if cmds.objExists(oldShapePath):

                cmds.delete(oldShapePath)

            # --------------------------------------------------
            # Parent new shape under controller
            # --------------------------------------------------

            newShape = cmds.parent(
                tempShape,
                ctrl,
                shape=True,
                relative=True
            ...
            # Rename shape to original name
            newShape = cmds.rename(
                newShape,
                curveChild
            )

            # --------------------------------------------------
            # Restore curve attributes
            # --------------------------------------------------

            try:
                cmds.setAttr(
                    newShape + ".overrideEnabled",
                    curveData["overrideEnabled"]
                )
            except:
                pass
            ...
            try:
                cmds.setAttr(
                    newShape + ".overrideRGBColors",
                    curveData["overrideRGBColors"]
                )
            except:
                pass
            ...
            try:
                cmds.setAttr(
                    newShape + ".visibility",
                    curveData["visibility"]
                )
            except:
                pass

            if curveData.get("overrideRGBColors") == 1:

                try:
                    cmds.setAttr(
                        newShape + ".drawOverride.overrideColorR",
                        curveData["overrideColorR"]
                    )

                    cmds.setAttr(
                        newShape + ".drawOverride.overrideColorG",
                        curveData["overrideColorG"]
                    )

                    cmds.setAttr(
                        newShape + ".drawOverride.overrideColorB",
                        curveData["overrideColorB"]
                    )
                except:
                    pass
                ...
                try:
                    cmds.setAttr(
                        newShape + ".overrideColor",
                        curveData["overrideColor"]
                    )
                except:
                    pass
            ...
            # If the temporary transform still exists,
            # delete it.
            if tempCurve and cmds.objExists(tempCurve):
                cmds.delete(tempCurve)


    def QuickImportCurve(data,*arr):
        ...
        filePath = data["sceneDataPath"] + "dataCurveShape.json"

        if not os.path.exists(filePath):
            print(
                "Curve data not found: {}".format(
                    filePath
                )
            )
            return

        dataTemp = NLTA_General.readJsonFile(
            filePath
        )
        ...
        for ctrl in ctrls:
            ...
            # ----------------------------------------------
            # Controller display attributes
            # ----------------------------------------------

            try:
                cmds.setAttr(
                    ctrl + ".overrideEnabled",
                    ctrlData["overrideEnabled"]
                )

                cmds.setAttr(
                    ctrl + ".overrideRGBColors",
                    ctrlData["overrideRGBColors"]
                )
            except:
                pass
            ...
            if ctrlData.get("overrideRGBColors") == 1:
                ...
                try:
                    cmds.setAttr(
                        ctrl + ".drawOverride.overrideColorR",
                        ctrlData["overrideColorR"]
                    )

                    cmds.setAttr(
                        ctrl + ".drawOverride.overrideColorG",
                        ctrlData["overrideColorG"]
                    )

                    cmds.setAttr(
                        ctrl + ".drawOverride.overrideColorB",
                        ctrlData["overrideColorB"]
                    )
                except:
                    pass
            ...
            for curveChild, curveChildData in curveData.items():

                curveChildPath = ctrl + "|" + curveChild

                pointData = curveChildData.get(
                    "pointData",
                    {}
                )
                ...
                if cmds.objExists(curveChildPath):
                    ...
                    currentPoints = cmds.ls(
                        curveChildPath + ".controlPoints[*]",
                        flatten=True
                    )
                    ...
                    if len(currentPoints) == len(savedPoints):
                        ...
                        try:
                            cmds.setAttr(
                                curveChildPath + ".overrideEnabled",
                                curveChildData["overrideEnabled"]
                            )

                            cmds.setAttr(
                                curveChildPath + ".overrideRGBColors",
                                curveChildData["overrideRGBColors"]
                            )
                        except:
                            pass
                        """
                        try:
                            cmds.setAttr(
                                curveChildPath + ".visibility",
                                curveChildData["visibility"]
                            )
                        except:
                            pass
                        """
                        if curveChildData.get(
                            "overrideRGBColors"
                        ) == 1:
                            ...
                            try:
                                cmds.setAttr(
                                    curveChildPath
                                    + ".drawOverride.overrideColorR",
                                    curveChildData["overrideColorR"]
                                )

                                cmds.setAttr(
                                    curveChildPath
                                    + ".drawOverride.overrideColorG",
                                    curveChildData["overrideColorG"]
                                )

                                cmds.setAttr(
                                    curveChildPath
                                    + ".drawOverride.overrideColorB",
                                    curveChildData["overrideColorB"]
                                )
                            except:
                                pass
                            ...
                            try:
                                cmds.setAttr(
                                    curveChildPath + ".overrideColor",
                                    curveChildData["overrideColor"]
                                )
                            except:
                                pass
                ...
                # ------------------------------------------
                    ...
                    print(
                        "Curve shape not found, rebuilding: {}".format(
                            curveChildPath
                        )
                    )
    ...
    cmds.button(label="Quick Export",c=partial(QuickExportCurve,data))
    cmds.button(label="Quick Import",c=partial(QuickImportCurve,data))
...
def Run(data,*arr):
    ...
    if datas:
        pm.mel.eval("AdvancedSkeleton;asControllerGalleryImport;")
        for data in datas:
...
def Add(listUI,data,*arr):
    ...
    shapeData = NLTA_Control.ShapeASData()
    for key in shapeData:
        cmds.menuItem(label=key)
    cmds.optionMenu(itemData["shape"], e=True, value=data.get("shape",shapeData[0]))
    cmds.text(label="")
