...
def Form(data,*args):
    ...
    cmds.button(label="Add",width=100,c=partial(Add,listUI,{}))
...
def callback():
    ...
    def CreateWindow(*arr):
        ...
        def AddKeySingle(data,*arr):
            node = [n for n in cmds.ls() if n.rsplit(":", 1)[-1] == "{nodeName}"][0]
            switchControlName = data["switchControl"]
            switchControl = [n for n in cmds.ls() if n.rsplit(":", 1)[-1] == switchControlName][0]
            cmds.setKeyframe(switchControl)
            sourcesArray = data["refObjects"].split("\n")
            for i in range(len(sourcesArray)):
                source = sourcesArray[i]
                sourceReal = [n for n in cmds.ls() if n.rsplit(":", 1)[-1] == source][0]
                attrs = ["tx", "ty", "tz", "rx", "ry", "rz"]
        ...
        def AddKeys(*arr):
            ...
            if objs:
                ...
                for snapData in snapDatas:
                    ...
                    for obj in objs:
                        obj = obj.split(":")[-1]
                        if obj in (snapData["refObjects"].split("\n")+[snapData["switchControl"]]):
                            works.append(snapData)
                for work in works:
                    AddKeySingle(work)
        ...
        #cmds.button(label="Add Keys",command=AddKeys,height=40)
        cmds.showWindow(win)
