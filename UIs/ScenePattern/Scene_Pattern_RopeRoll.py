...
def Run(data,*arr):
    ...
    if datas:
        for i in range(len(datas)):
            ...
            activeCountPma = cmds.createNode("plusMinusAverage",n="{}_{}_activeCount".format(attrContent, attr))
            cmds.setAttr(activeCountPma + ".operation",2)
            cmds.setAttr(activeCountPma + ".input1D[0]",objsCount)
            cmds.connectAttr(offsetPma + ".output1D",activeCountPma + ".input1D[1]")

            plus = cmds.createNode("plusMinusAverage",n="{}_{}_plus".format(attrContent, attr))
            cmds.setAttr(plus + ".operation",1)
            cmds.connectAttr(activeCountPma + ".output1D",plus + ".input1D[0]")
            ...
            for i, (obj, dest) in enumerate(zip(objs, dests), start=1):
                ...
                cmds.connectAttr(activeCountPma + ".output1D",cond + ".firstTerm")
                ...
                cmds.connectAttr(activeCountPma + ".output1D",startPma + ".input1D[0]")
...
def Add(listUI,data,*arr):
    ...
    itemData["offset"] = cmds.textField(text=data.get("offset", ""))
    cmds.text(label="")
