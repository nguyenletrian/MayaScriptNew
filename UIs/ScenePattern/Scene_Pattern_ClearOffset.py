        ...
        if objs:
            for obj in objs:
                if cmds.objExists(obj):
                    parent = cmds.listRelatives(obj, parent=True, f=True)[0]
                    zeroExtra =  cmds.group(em=True, name="{}_ClearOffsetGrp".format(obj))
                    cmds.delete(cmds.parentConstraint(obj,zeroExtra, mo=False))
                    cmds.parent(zeroExtra,parent)
                    cmds.parent(obj,zeroExtra)
    cmds.warning("Clear Offset done!~")
